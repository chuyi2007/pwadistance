...
public class Block implements Value {
    ...
    public void setScore(short[][] score, boolean isTranspose) {
        ...
        this.rowSize = score.length;
        this.colSize = score[0].length;
    }
	...
	public short[][] getScoreNormal() {
		return scoreNormal;
	}

	public void setScoreNormal(short[][] scoreNormal) {
		this.scoreNormal = scoreNormal;
	}

	public short[][] getScoreNormalReverse() {
		return scoreNormalReverse;
	}

	public void setScoreNormalReverse(short[][] scoreNormalReverse) {
		this.scoreNormalReverse = scoreNormalReverse;
	}
    ...
    public void fromBytes(byte[] bytes) throws SerializationException {
        ...
        try {
            ...
            if (din.available() > 0) {
                ...
                this.scoreNormal = new short[rows][cols];
                for (int i = 0; i < rows; i++) {
                    for (int j = 0; j < cols; j++) {
                        this.scoreNormal[i][j] = din.readShort();
                    }
                }
                
                this.scoreNormalReverse = new short[rows][cols];
                for (int i = 0; i < rows; i++) {
                    for (int j = 0; j < cols; j++) {
                        this.scoreNormalReverse[i][j] = din.readShort();
                    }
                }
            }
            din.close();
    ...
    public byte[] getBytes() throws SerializationException {
        ...
        try {
            ...
            if (score != null) {
                ...
                for (short[] row : scoreNormal) {
                    for (int i : row) {
                        dout.writeShort(i);
                    }
                }
                
                for (short[] row : scoreNormalReverse) {
                    for (int i : row) {
                        dout.writeShort(i);
                    }
                }
