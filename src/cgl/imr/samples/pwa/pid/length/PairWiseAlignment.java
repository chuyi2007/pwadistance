...
public class PairWiseAlignment {
	...
	public static double drivePWAMapReduce(int numOfMapTasks, int numOfReduceTasks,
			...
			String geneBlockPrefix, String outputPrefix, String outputFile, String type, String lengthPrefix,
			String reverseFlag)
	throws TwisterException {
		...
		jobConf.addProperty("lengthPrefix", lengthPrefix);
		jobConf.addProperty("ReverseFlag", reverseFlag);
	...
	public static void main(String args[]) {
		...
		if (args.length != 11) {
			...
			"[num_of_partitions] [data_dir] [gene_block_prefix] [tmp_output_prefix] [output_map_file] [type]" +
			"[alignment_length_output_prefix][reverse flag (0:no, 1:yes)");
			System.exit(2);
		}
		...
		String type = args[8];
		String lengthPrefix = args[9];
		String reverseFlag = args[10];
		//String idxFile = args[9];
		...
		if( !reverseFlag.equals("0") && !reverseFlag.equals("1")){
			System.err.println("The reverse flag must be 1 or 0");
			System.exit(2);
		}
		...
		try {
			sequentialTime = drivePWAMapReduce(numOfMapTasks, numOfReduceTasks, numOfSeqs,
					numOfPartitions, dataDir, geneBlockPrefix, outputPrefix, outputFile, type, lengthPrefix, 
					reverseFlag);
