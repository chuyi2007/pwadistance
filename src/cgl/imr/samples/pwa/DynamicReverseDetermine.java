...
public class DynamicReverseDetermine implements Runnable{
	...
	public DynamicReverseDetermine(int row) throws IOException{
		this.row = row;
	...
	public void run(){
		...
		for(int j = 0; j < totalSize; j++){
			if(sampleSet.contains(j)){
				Sequence sequenceA = sequences.get(this.row);
				Sequence sequenceB = sequences.get(j);
				try {
	...
	public static void main(String[] args) throws Exception {
		...
		sequences = SequenceParser.parse(inputFastaFile, Alphabet.DNA);
		...
		for(int i = 0; i < totalSize; i++)
			futures.add(pool.submit(new DynamicReverseDetermine(i)));
		for(int i = 0; i < totalSize; i++)
			futures.get(i).get();
		pool.shutdown();
