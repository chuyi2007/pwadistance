...
public class PWAMapTask implements MapTask {
	...
	throws TwisterException {
		...
		for (Block block : blocks) {
			...
			short[][] lengthNoEndGap = new short[rowSize][columnSize];
			short[][] length = new short[rowSize][columnSize];
			short[][] identicalPairs = new short[rowSize][columnSize];
			short[][] distances = new short[rowSize][columnSize];
			
			double[] rowScore = new double[rowSize];
			double[] columnScore = new double[columnSize];
			...
			if(type.equals("NW")){
				...
				try {
					...
					for(int j = 0; j < rowSize; j++){
						...
						rowScore[j] = ad.getScore();
					
					}
					
					for(int j = 0; j < columnSize; j++){
						Sequence sequenceA = colBlockSequences[j];
						List<AlignedData> ads = aligner.align(ednafull, -gapOpen, -gapExt, 
								sequenceA, sequenceA);
						// We will just take the first alignment
						AlignedData ad = ads.get(0);
						columnScore[j] = ad.getScore();
					}
					...
					if (rowBlockNumber != columnBlockNumber) {
						...
						for (int j = 0; j < rowSize; j++) {
							for (int k = 0; k < columnSize; k++) {
								...
								List<AlignedData> ads = aligner.align(ednafull, -gapOpen, -gapExt, 
										sequenceA, sequenceB);
								AlignedData ad = ads.get(0);
								int tmpScore = ad.getScore();
								short tmpLength = ad.getAlignmentLength();
								short tmpLengthNoEndGap = ad.getAlignmentLengthExcludingEndGaps();
								...
								if(seqType.equals("DNA")){
									...
									if(tmpScore < ad.getScore()){
										...
										tmpLengthNoEndGap = ad.getAlignmentLengthExcludingEndGaps();
								...
								lengthNoEndGap[j][k] = tmpLengthNoEndGap;
								identicalPairs[j][k] = tmpIdenticalPairs;
								double distanceDouble = 2.0 * ((double) tmpScore) /(rowScore[j] + columnScore[k]);
								distances[j][k] = (short)((1 - distanceDouble) * Short.MAX_VALUE);
						...
						for (int j = 0; j < rowSize; j++) {
							for (int k = 0; k < j; k++) {
								...
								lengthNoEndGap[j][k] = tmpLengthNoEndGap;
								lengthNoEndGap[k][j] = lengthNoEndGap[j][k];
								identicalPairs[j][k] = tmpIdenticalPairs;
							...
							lengthNoEndGap[j][j] = ad.getAlignmentLengthExcludingEndGaps();
							length[j][j] = ad.getAlignmentLength();
							identicalPairs[j][j] = ad.getNumberOfIdenticalBasePairs(false);
							double distanceDouble = 2.0 * ((double)score[j][j]) /(rowScore[j] + rowScore[j]);
							//System.out.println("ad: " + ad.getScore() +  " rowScore[j]" + rowScore[j]);
							distances[j][j] = (short)((1 - distanceDouble) * Short.MAX_VALUE);
							//System.out.println("double: " + distanceDouble + "distancejj: " + distances[j][j]);
						}
					}
				...
				block.setLengthNoEndGap(lengthNoEndGap);
				block.setIdenticalPairs(identicalPairs);
				block.setDistances(distances);
				...
				if (rowBlockNumber != columnBlockNumber) {
					...
					transBlock.setLengthNoEndGap(block.getLengthNoEndGap());
					transBlock.setIdenticalPairs(block.getIdenticalPairs());
					transBlock.setDistances(block.getDistances());
					try {
